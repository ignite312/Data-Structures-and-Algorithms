...
int ip1, ip2;
array<int, 2> p_ip1[N], p_ip2[N];

void init() {
    p_ip1[0] = p_ip2[0] = {1, 1};
    for(int i = 1; i < N; i++) {
        p_ip1[i][0] = p_ip1[i-1][0] *1LL* p1 % M1;
        p_ip2[i][0] = p_ip2[i-1][0] *1LL* p2 % M2;
    }
    ip1 = bigPow(p1, M1-2, M1);
    ip2 = bigPow(p2, M2-2, M2);
    for(int i = 1; i < N; i++) {
        p_ip1[i][1] = p_ip1[i-1][1]*1LL* ip1 % M1;
        p_ip2[i][1] = p_ip2[i-1][1] *1LL* ip2 % M2;
    }
}
